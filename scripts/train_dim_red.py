...
from src.fcnf.network import SIRENautoencoder, TransolverAutoencoder, MLPautoencoder, PerceiverAutoencoder
from src.fcnf.factorized_autoencoder import SpatiotemporalAutoencoder
from src.engine.trainer_fcnf import FAETrainer
from src.utils.data import get_data, get_dataloader
...
def get_wandb_run_name(cfg: DictConfig) -> str:
    ...
    #               f"Nl{cfg.model.num_latents}",
    #               f"E{cfg.model.encoder_n_layers}x{cfg.model.encoder_hidden_features}",
    #               f"D{cfg.model.decoder_n_layers}x{cfg.model.decoder_hidden_features}",]
    ...
    basic_info = ["STautoencoder"]
    model_params = basic_info + quantizer_info
    
    # Combine all parts
    return f"[DR]{dataset_name}_{'_'.join(model_params)}"
...
def train(cfg: DictConfig):
    ...
    # load data
    train_dataset = get_data(train_val_flag="train", **cfg.data)
    data_kwargs = OmegaConf.to_container(cfg.data, resolve=True) # plain dict
    data_kwargs.pop("masking_strategy")
    val_dataset = get_data(train_val_flag="val", 
    ...
    # Create model
    model = SpatiotemporalAutoencoder(**cfg.model)
